...
# from model.model import get_open_ai_json,get_open_ai

#from langgraph.checkpoint.sqlite import SqliteSaver
from langgraph_module. agent import (
    PlannerAgent,
    SelectorAgent,
    ReporterAgent,
    ReviewerAgent,
...
from tool.google_serper import google_serper
from tool.scraper import scrape_website
from langgraph_module. state import AgentGraphState, get_agent_graph_state, state
...
def create_graph(server=None, model=None, stop=None, model_endpoint=None, temperature=0):
    ...
    graph.add_node("end_node", lambda state: EndNodeAgent(state).invoke())
    ...
    graph.add_edge("final_report", "end_node")
